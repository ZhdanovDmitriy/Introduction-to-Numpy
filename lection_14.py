# Ансамблевые методы (Ensemble methods) — классификация и регрессия
# Комбинируют несколько слабых моделей для получения сильной
# Вместо одной модели создаётся ансамбль (набор) моделей, чьи предсказания объединяются (например, голосованием или усреднением)

# Достоинства
# - Повышение качества модели за счёт объединения нескольких слабых моделей
# - Уменьшение переобучения (особенно в методах типа Bagging, например Random Forest)
# - Гибкость: можно использовать разные алгоритмы в ансамбле или разные параметры одних и тех же моделей
# - Хорошо работают с высокоразмерными и разнородными данными

# Недостатки
# - Высокие вычислительные затраты при обучении, особенно при большом числе базовых моделей
# - Сложность интерпретации: итоговое решение формируется коллективно, и сложно понять вклад отдельной модели
# - Возможна переусложнённость, если ансамбль собран без должного контроля (особенно в Boosting)
# - Зависимость от выбора типа базовых моделей и параметров ансамблирования
